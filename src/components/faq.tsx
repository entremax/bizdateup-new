...
const FrequentlyAsked: React.FC<{custom?:boolean,faqData?: { _id:string,question:string,answer:string }[]}> = ({custom=false,faqData}) => {
  ...
  function convertToCollapseFormat(collapseData?: { _id:string,question:string,answer:string }[]) {
  ...
  return (
    <Collapse
      ...
      items={custom?convertToCollapseFormat(faqData):getRandomFourItems({array: data.faqData})}
      bordered={false}
      defaultActiveKey={['1']}
      size={"large"}
      style={{
