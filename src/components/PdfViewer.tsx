...
const PdfViewer = () => {
  ...
  console.log("🚀 ~ file: PdfViewer.tsx:6 ~ PdfViewer ~ base64Images:", base64Images)

  useEffect(() => {
    const fetchBase64Images = async () => {
      try {
        const response = await fetch('https://www.bizdateup.com/v0/pitch_viewer/2023-06-27T14-49-30.700Z-BIZ_DATE_UP_PITCH_compressed.pdf');
  ...
  {base64Images?.map((imageObject, index) => (
    <div key={index}>
      <Image src={`data:image/png;base64, ${imageObject.base64}`} fill
