...
export default function StartupCard({startup}:{startup:StartupData|StartupDataByType}){
  return (
    // <div className="grid gap-6 grid-cols-1 lg:grid-cols-2 justify-center py-6 ">
    //   {getData(data).map((startup) => (
        <div
          key={startup._id}
          className={"relative w-full md:w-[24rem] rounded-2xl border_gray overflow-clip shadow"}
        >
          <Image
          ...
          <div className={"p-5 pt-8 grid gap-4"}>
            ...
            <p className={"reset text-sm text-[#828F99] text-ellipsis leading-normal line-clamp-3"}>{startup.shortDescription}</p>
