...
const StickyCompanyIntro: React.FC<{ startup: StartupData }> = ({ startup }) => {
  ...
  const staticClass = "top-[4.56rem] left-0 right-0  grid grid-cols-12  gap-2 z-[999] bg-white shadow-lg";
    ...
    <div className={isFixed ? "sticky " + staticClass : "hidden"}>
      <div className="flex justify-between items-center col-span-full xl:col-start-2 xl:col-end-12">
        <div className="flex items-center gap-4 col-start-2">
          ...
          <h3
            className="text-2xl font-bold leading-normal lg:leading-[4rem] text-primary-dark reset">
            {capitalizeFirstLetter(startup.registeredCompanyName.trim().split(" "))}
          </h3>
        </div>
        <div className="md:pl-8 xl:px-12 max-w-[17rem]">
          <Button type={"default"} className={"!bg-primary !text-white "} size={"large"} block>I am ready to Invest</Button>
        </div>
      </div>
