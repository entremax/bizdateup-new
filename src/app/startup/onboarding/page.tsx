...
export default function OnboardingForm() {
  ...
  const textAreaRefs={
    'previous-round': React.useRef<TextAreaRef | null>(null),
    'describe-product': React.useRef<TextAreaRef | null>(null),
    traction: React.useRef<TextAreaRef | null>(null),
  }
  const getReferCode = () => {
    // if (user?.refer) {
    //   return user?.refer
  ...
  console.log(refs)
  const handleSubmit = () => {
    let values1: { [key in keyof typeof refs]: unknown | null } = {} as {
    ...
    let values2: { [key in keyof typeof textAreaRefs]: unknown | null } = {} as {
      [key in keyof typeof textAreaRefs]: unknown | null
    }
    for (let key in textAreaRefs) {
      //@ts-ignore
      values2[key] = textAreaRefs[key]?.current?.resizableTextArea?.textArea.value??''
    }
          ...
          {inputFields.map((field, index) =>
            field.fieldType === 'select' && 'options' in field ? (
              ...
              <TextAreaInput
                ...
                //@ts-ignore
                ref={field.fieldType !== 'select' && textAreaRefs[field.name]}
                name={field.name}
                label={field.label}
