...
import { PickerRef } from 'antd/lib/date-picker';
import { ForwardRefProps } from '@/types/profile';
import { DatePickerProps } from 'antd/lib/date-picker';
// import { PickerLocale } from 'rc-picker/lib/interface';
...
const DatePicker: ForwardRefRenderFunction<PickerRef, PickerProps> = (
  {
    wrapperClassName,
    label,
    name,
...
const changeHandler: DatePickerProps['onChange'] = (date, dateString:string) => {
  ...
  const defaultTime = defaultTimeValue && name == "date"
  ? dayjs(defaultTimeValue).format(dateFormat) : defaultTimeValue && name == "time"?defaultTimeValue
  : undefined;

  console.log("🚀 ~ file: DatePicker.tsx:31 ~ defaultTimeValue:", defaultTimeValue)
  console.log("🚀 ~ file: DatePicker.tsx:45 ~ defaultTime:", defaultTime)
  return (
    <div
      className={cn(
        'relative w-full ' + wrapperClassName,
      )}
    >
      <AntDDatePicker
        // name={name}
